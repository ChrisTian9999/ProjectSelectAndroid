...
public class ProjectDataHttpRequest extends HttpRequest {
     ...
     * 创建课题
     */
    public void createProject(Subscriber<BaseResponse<SimpleFlagEntity>> subscriber, int departmentId, int majorId, int teacherId, String title, String detail, int ranking) {
        ProjectEntity projectEntity = new ProjectEntity(departmentId, majorId, teacherId, title, detail, ranking);
        Observable<BaseResponse<SimpleFlagEntity>> observable = mProjectDataService.createProject(projectEntity);
